...
class FakturekspedisiController extends Controller
{
    public function index()
    {
        ...
        return view('admin.faktur_ekspedisi.index', compact('pelanggans', 'memos', 'tarifs', 'memoEkspedisi', 'memoTambahan'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('memo_ekspedisi_id') || $request->has('kode_memo') || $request->has('nama_driver') || $request->has('telp_driver') || $request->has('nama_rute') || $request->has('kendaraan_id') || $request->has('no_kabin') || $request->has('no_pol')) {
            for ($i = 0; $i < count($request->memo_ekspedisi_id); $i++) {
                ...
                $kode_memotambahans = $request->kode_memotambahans[$i] ?? null;
                $tanggal_memotambahans = $request->tanggal_memotambahans[$i] ?? null;
                $nama_drivertambahans = $request->nama_drivertambahans[$i] ?? null;
                $nama_rutetambahans = $request->nama_rutetambahans[$i] ?? null;
        ...
        $cetakpdf = Faktur_ekspedisi::create([
            ...
            'qrcode_faktur' => 'https://batlink.id/faktur_ekspedisi/' . $kode,
            'status' => 'posting',
            'status_notif' => false,
        ]);
