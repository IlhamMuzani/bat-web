...
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item active">Inquery Nota Return Barang</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Data Inquery Nota Return Barang</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($inquery as $nota)
                                <tr id="editMemoekspedisi" data-toggle="modal"
                                    ...
                                    <td>
                                        {{ number_format($nota->grand_total, 0, ',', '.') }}
                                ...
                                <div class="modal fade" id="modal-posting-{{ $nota->id }}">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <p>Memo ekspedisi
                                                    <strong>{{ $nota->kode_return }}</strong>
                                                </p>
                                                @if ($nota->status == 'unpost')
                                                    @if (auth()->check() && auth()->user()->fitur['inquery return nota barang delete'])
                                                        <form method="GET"
                                                            action="{{ route('hapusnota', ['id' => $nota->id]) }}">
                                                            <button type="submit"
                                                    ...
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['inquery return nota barang show'])
                                                        <a href="{{ url('admin/inquery_notareturn/' . $nota->id) }}"
                                                            type="button" class="btn btn-outline-info btn-block">
                                                            <i class="fas fa-eye"></i> Show
                                                        </a>
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['inquery return nota barang update'])
                                                        <a href="{{ url('admin/inquery_notareturn/' . $nota->id . '/edit') }}"
                                                            type="button" class="btn btn-outline-warning btn-block">
                                                            <i class="fas fa-edit"></i> Update
                                                        </a>
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['inquery return nota barang posting'])
                                                        <form method="GET"
                                                            action="{{ route('postingnota', ['id' => $nota->id]) }}">
                                                            <button type="submit"
                                                ...
                                                @if ($nota->status == 'posting')
                                                    ...
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['inquery return nota barang unpost'])
                                                        <form method="GET"
                                                            action="{{ route('unpostnota', ['id' => $nota->id]) }}">
                                                            <button type="submit"
    ...
    <script>
        ...
        function cari() {
            form.action = "{{ url('admin/inquery_notareturn') }}";
            form.submit();
        }
    </script>
    ...
    {{-- <script>
        document.getElementById('editMemoekspedisi').addEventListener('dblclick', function() {
            window.location.href = "{{ url('admin/inquery_notareturn/' . $nota->id . '/edit') }}";
        });
    </script> --}}
