...
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ url('admin/inquery_buktipotongpajak') }}">Inquery Bukti Potong
                                Pajak</a></li>
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            @endif
            <form action="{{ url('admin/inquery_buktipotongpajak/' . $buktipotongpajak->id) }}" method="POST"
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" placeholder="masukkan nomor penerimaan"
                                            value="{{ old('nomor_faktur', $buktipotongpajak->nomor_faktur) }}"
                                            name="nomor_faktur">
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    ...
                                    <div class="input-group date" id="reservationdatetime">
                                        <input style="font-size:14px" type="date" id="periode_awal" name="periode_awal"
                                            placeholder="d M Y sampai d M Y"
                                            data-options='{"mode":"range","dateFormat":"d M Y","disableMobile":true}'
                                            value="{{ old('periode_awal', $buktipotongpajak->periode_awal) }}"
                            ...
                            <table id="example1" class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach ($tagihan_ekspedisis as $tagihan)
                                        @php
                                            if (in_array($tagihan->id, array_keys($detail_id_data))) {
                                                $detail_id = $detail_id_data[$tagihan->id];
                                            } else {
                                                $detail_id = 0;
                                            }
                                        @endphp
    ...
    {{-- <script>
        ...
        function set_item(urutan, data, is_session = false) {
            ...
            col += '<td hidden>';
            col += '<input type="hidden" class="form-control" name="id[' + data.id + ']" value="' + data.id + '">';
            col += data.id;
            col += '</td>';
            col += '<td>';
            col += '<input type="hidden" class="form-control" name="kode_tagihan[' + data.id + ']" value="' + data
                .kode_tagihan + '">';
            col += kode_tagihan;
            col += '</td>';
            col += '<td>';
            col += '<input type="hidden" class="form-control" name="tanggal[' + data.id + ']" value="' + data.tanggal +
                '">';
            col += tanggal;
            col += '</td>';
            col += '<td>';
            col += '<input type="hidden" class="form-control" name="nama_pelanggan[' + data.id + ']" value="' + data
                .nama_pelanggan + '">';
            col += nama_pelanggan;
            col += '</td>';
            ...
            col += '<input type="hidden" class="form-control pph" id="pph-' + data.id + '" name="pph[' + data.id +
                ']" value="' + pph + '">';
            col += '</td>';
            col += '<td class="text-right">';
        ...
        function delete_item(id, detail_id) {
            ...
            if (detail_id !== 0) {
                $.ajax({
                    url: "{{ url('admin/inquery_buktipotongpajak/delete_item') }}" + '/' + detail_id,
                    type: "GET",
                    dataType: "json",
                    success: function(data) {
        ...
        function set_grand_total() {
            ...
            $('#tabel-barang tr').each(function() {
                var totalText = $(this).find('td').eq(5).text().replace(/[^,\d]/g,
