        ...
        <div class="container-fluid">
            ...
            @if (session('error'))
                <div class="alert alert-danger alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <h5>
                        <i class="icon fas fa-Part"></i> Error!
                    </h5>
                    {{ session('error') }}
                </div>
            @endif
                        ...
                        <tbody>
                            @foreach ($inquery as $fakturpelunasan)
                                <tr id="editMemoekspedisi" data-toggle="modal"
                                    ...
                                    <td>{{ $fakturpelunasan->kode_pelunasanpart }}</td>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                @if ($fakturpelunasan->status == 'unpost')
                                                    @if (auth()->check() && auth()->user()->fitur['delete pelunasan faktur pembelian part'])
                                                        <form method="GET"
                                                            action="{{ route('hapuspelunasanpart', ['id' => $fakturpelunasan->id]) }}">
                                                            <button type="submit"
                                                    ...
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['show pelunasan faktur pembelian part'])
                                                        <a href="{{ url('admin/inquery_partpembelianlunas/' . $fakturpelunasan->id) }}"
                                                            type="button" class="btn btn-outline-info btn-block">
                                                            <i class="fas fa-eye"></i> Show
                                                        </a>
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['update pelunasan faktur pembelian part'])
                                                        <a href="{{ url('admin/inquery_partpembelianlunas/' . $fakturpelunasan->id . '/edit') }}"
                                                            type="button" class="btn btn-outline-warning btn-block">
                                                            <i class="fas fa-edit"></i> Update
                                                        </a>
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['posting pelunasan faktur pembelian part'])
                                                        <form method="GET"
                                                            action="{{ route('postingpelunasanpart', ['id' => $fakturpelunasan->id]) }}">
                                                            <button type="submit"
                                                ...
                                                @if ($fakturpelunasan->status == 'posting')
                                                    ...
                                                    @endif
                                                    @if (auth()->check() && auth()->user()->fitur['unpost pelunasan faktur pembelian part'])
                                                        <form method="GET"
                                                            action="{{ route('unpostpelunasanpart', ['id' => $fakturpelunasan->id]) }}">
                                                            <button type="submit"
