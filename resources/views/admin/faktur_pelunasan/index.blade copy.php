...
@section('content')
    ...
    $('#kategori').on('change', function() {
        ...
        if (selectedValue === "PPH") {
            ...
            $('#totalpembayaran').val(formatRupiah(grandTotal)); // Perbarui nilai total pembayaran sesuai grand total
        }
    });
});

function updateTotalPembayaran() {
    var selectedValue = $('#kategori').val(); // Dapatkan nilai kategori yang dipilih
    var grandTotal = parseFloat($('#grandTotalCell').text().replace(/\D/g, '')) || 0; // Ambil grand total saat ini
