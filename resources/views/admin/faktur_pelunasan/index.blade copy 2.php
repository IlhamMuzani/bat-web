...
@section('content')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <label style="font-size:14px" class="form-label" for="kategoris">Pilih Kategori</label>
                            <select style="font-size:14px" class="form-control" id="kategoris" name="kategoris">
                                <option value="">- Pilih -</option>
                                <option value="PPH" {{ old('kategoris') == 'PPH' ? 'selected' : null }}>
                                    PPH</option>
                                <option value="NON PPH" {{ old('kategoris') == 'NON PPH' ? 'selected' : null }}>
                                    NON PPH</option>
                            </select>
                        </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button type="button" class="btn btn-primary btn-sm"
                                                ...
                                                '{{ $return->detail_tagihan->pluck('total')->implode(', ') }}',
    ...
    <script>
        ...
        function GetReturn(Return_id, KodeReturn, Pelanggan_id, KodePelanggan, NamaPell, Telpel, AlamatPelanggan,
            ...
            // Tambahkan event listener untuk memantau perubahan pada pilihan kategori
        ...
        $(document).ready(function() {
            $('#kategoris').on('change', function() {
                var selectedValue = $(this).val(); // Dapatkan nilai kategori yang dipilih
                var grandTotal = parseFloat($('#grandTotalCell').text().replace(/\D/g, '')) ||
                    0; // Ambil grand total saat ini

                if (selectedValue === "PPH") {
                    var pph = 0.02 * grandTotal; // Hitung PPH
                    var subtotal = grandTotal - pph; // Hitung subtotal setelah dikurangi PPH
                    $('#totalpembayaran').val(formatRupiah(subtotal)); // Perbarui nilai total pembayaran
                    updateGrandTotal();
                    updateTotalPembayaran();
                    updateSubTotalsxx();
                    updateSubTotal();
                } else {
                    $('#totalpembayaran').val(formatRupiah(
                        grandTotal)); // Perbarui nilai total pembayaran sesuai grand total
                    updateGrandTotal();
                    updateTotalPembayaran();
                    updateSubTotalsxx();
                    updateSubTotal();
        ...
        function updateTotalPembayaran() {
            var selectedValue = $('#kategoris').val(); // Dapatkan nilai kategori yang dipilih
            var grandTotal = parseFloat($('#grandTotalCell').text().replace(/\D/g, '')) || 0; // Ambil grand total saat ini
