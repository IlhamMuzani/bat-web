...
@section('content')
                            ...
                            <div class="row">
                                <div class="col-md-2 mb-3">
                                    ...
                                    <button type="button" class="btn btn-success btn-block mt-1" id="postingfilter"
                                        ...
                                        <i class="fas fa-check-circle"></i> Posting Filter
                                    </button>
                                            ...
                                            @if ($pengeluaran->status == 'unpost')
                                                ...
                                                @if (auth()->check() && auth()->user()->fitur['inquery pengambilan kas kecil update'])
                                                    <a class="dropdown-item"
                                                        href="{{ url('admin/inquery_pengeluarankaskecil/' . $pengeluaran->id . '/edit') }}"
                                                        oncontextmenu="window.location.href='{{ url('admin/inquery_pengeluarankaskecil/' . $pengeluaran->id . '/edit') }}'; return false;">
                                                        Update
                                                    </a>
                                                @endif
                                                @if (auth()->check() && auth()->user()->fitur['inquery pengambilan kas kecil show'])
                                                    <a class="dropdown-item"
                                                        href="{{ url('admin/inquery_pengeluarankaskecil/' . $pengeluaran->id) }}"
                                                        oncontextmenu="window.location.href='{{ url('admin/inquery_pengeluarankaskecil/' . $pengeluaran->id) }}'; return false;">Show</a>
                                                @endif
                                                @if (auth()->check() && auth()->user()->fitur['inquery pengambilan kas kecil delete'])
                                                    <form style="margin-top:5px" method="GET"
                                                        action="{{ route('hapusmemo', ['id' => $pengeluaran->id]) }}">
                                                        <button type="submit"
    ...
    <script>
        $(document).ready(function() {
            $('.unpost-btn').on('contextmenu', function(e) { // Mengubah event menjadi 'contextmenu'
                e.preventDefault();
    ...
    <script>
        $(document).ready(function() {
            $('.posting-btn').on('contextmenu', function(e) { // Mengubah event menjadi 'contextmenu'
                e.preventDefault();
    ...
    <script>
        ...
        function printSelectedData() {
            ...
            if (selectedIds.length === 0) {
                ...
                window.location.href = "{{ url('admin/cetak_pengeluaranfilter') }}?ids=" + selectedIdsString;
    ...
    <script>
        $(document).ready(function() {
            $('tbody tr.dropdown').on('click', function(e) {
                ...
                dropdownMenu.css({
                    'position': 'absolute',
                    'top': tdPosition.top + clickedTd.height() + 10,
