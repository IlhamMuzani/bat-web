...
<head>
    ...
    <title>Laporan Pelunasan Pembelian Ban</title>
    <style>
        html,
        body {
...
<body style="margin: 0; padding: 0;">
    ...
    <div style="font-weight: bold; text-align: center">
        <span style="font-weight: bold; font-size: 18px;">LAPORAN PELUNASAN FAKTUR PEMBELIAN BAN - RANGKUMAN</span>
        <br>
        <div class="text">
            @php
        ...
        @foreach ($inquery as $faktur)
            <tr>
                ...
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->pembelian_ban)
                        {{ $faktur->pembelian_ban->kode_pembelian_ban }}
                    @else
                        tidak ada
                    @endif
                </td>
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->pembelian_ban)
                        {{ $faktur->pembelian_ban->tanggal_awal }}
                    @else
                        tidak ada
                    @endif
                </td>
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->pembelian_ban)
                        {{ $faktur->pembelian_ban->user->karyawan->nama_lengkap }}
                    @else
                        tidak ada
                    @endif
                ...
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->faktur_pelunasanban)
                        {{ $faktur->faktur_pelunasanban->nama_supplier }}
                    @else
                        tidak ada
                    @endif
                </td>
