...
<body style="margin: 0; padding: 0;">
    ...
    <div style="font-weight: bold; text-align: center">
        <span style="font-weight: bold; font-size: 22px;">LAPORAN PELUNASAN EKSPEDISI - RANGKUMAN</span>
        <br>
        <div class="text">
            @php
    ...
    <table style="width: 100%; border-top: 1px solid black;" cellpadding="2" cellspacing="0">
        ...
        @foreach ($inquery as $faktur)
            <tr>
                ...
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">{{ $faktur->kode_faktur }}
                </td>
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    {{ $faktur->tanggal_faktur }}
                </td>
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->faktur_pelunasan)
                        {{ $faktur->faktur_pelunasan->user->karyawan->nama_lengkap }}
                    @else
                        tidak ada
                    @endif
                ...
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->faktur_pelunasan)
                        {{ $faktur->faktur_pelunasan->nama_pelanggan }}
                    @else
                        tidak ada
                    @endif
                </td>
