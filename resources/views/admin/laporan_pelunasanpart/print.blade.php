...
<head>
    ...
    <title>Laporan Pelunasan Pembelian Part</title>
    <style>
        html,
        body {
...
<body style="margin: 0; padding: 0;">
    ...
    <div style="font-weight: bold; text-align: center">
        <span style="font-weight: bold; font-size: 18px;">LAPORAN PELUNASAN FAKTUR PEMBELIAN PART - RANGKUMAN</span>
        <br>
        <div class="text">
            @php
        ...
        @foreach ($inquery as $faktur)
            <tr>
                ...
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->pembelian_part)
                        {{ $faktur->pembelian_part->kode_pembelianpart }}
                    @else
                        tidak ada
                    @endif
                </td>
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->pembelian_part)
                        {{ $faktur->pembelian_part->tanggal_awal }}
                    @else
                        tidak ada
                    @endif
                </td>
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->pembelian_part)
                        {{ $faktur->pembelian_part->user->karyawan->nama_lengkap }}
                    @else
                        tidak ada
                    @endif
                ...
                <td class="td" style="text-align: left; padding: 5px; font-size: 12px;">
                    @if ($faktur->faktur_pelunasanpart)
                        {{ $faktur->faktur_pelunasanpart->nama_supplier }}
                    @else
                        tidak ada
                    @endif
                </td>
